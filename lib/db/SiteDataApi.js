...
import hydrateCollectionQuery from '@/lib/db/notion/hydrateCollectionQuery'
...
const safeIdToUuid = id => {
  if (typeof id !== 'string' || !id) return null
  try {
    if (checkStrIsUuid(id)) return id
    if (checkStrIsNotionId(id)) return idToUuid(id)
  } catch (error) {
    console.warn('[safeIdToUuid] invalid id:', id, error)
  }
  return null
}
...
export async function resolvePostProps({
  ...
  post = props?.allPages?.find(p => {
    ...
    const postIdUuid = safeIdToUuid(postId)
    const fullSlugUuid = safeIdToUuid(fullSlug)

    return (
      slugCandidates.has(p.slug) ||
      slugCandidates.has(p.slug?.split('/').pop()) ||
      (postId && slugCandidates.has(postId)) ||
      (postIdUuid && slugCandidates.has(postIdUuid)) ||
  ...
  if (post && !post?.blockMap) {
    try {
      ...
      const postIdUuid = safeIdToUuid(postId || '')
      if (!postIdUuid) {
        console.warn('[resolvePostProps] skip fetchNotionPageBlocks for invalid post.id:', post.id)
        post.blockMap = { block: {} }
      } else {
        const rawBlockMap = await fetchNotionPageBlocks(postIdUuid, source)
      ...
      post.blockMap = post.blockMap || { block: {} }
    }
  }
...
async function convertNotionToSiteData(SITE_DATABASE_PAGE_ID, from, pageRecordMap) {
  ...
  const siteDatabasePageIdUuid = safeIdToUuid(SITE_DATABASE_PAGE_ID)
  if (!siteDatabasePageIdUuid) {
    console.error('Invalid NOTION_PAGE_ID:', SITE_DATABASE_PAGE_ID)
    return EmptyData(String(SITE_DATABASE_PAGE_ID || ''))
  }
  SITE_DATABASE_PAGE_ID = siteDatabasePageIdUuid
  ...
  const collectionIdUuid = safeIdToUuid(collectionId)
  const rawCollection =
    pageRecordMap.collection?.[collectionId] ||
    (collectionIdUuid ? pageRecordMap.collection?.[collectionIdUuid] : null) ||
    {}
  ...
  const hydratedCollection = await hydrateCollectionQuery({
    collectionId,
    collectionQuery,
    collectionView,
    viewIds,
  ...
  collectionQuery = hydratedCollection.collectionQuery
  block = Object.assign({}, block, hydratedCollection.recordMap?.block)
  pageRecordMap.collection = Object.assign(
    {},
    pageRecordMap.collection,
    hydratedCollection.recordMap?.collection
  )
  pageRecordMap.collection_view = Object.assign(
    {},
    pageRecordMap.collection_view,
    hydratedCollection.recordMap?.collection_view
  )
  pageRecordMap.notion_user = Object.assign(
    {},
    pageRecordMap.notion_user,
    hydratedCollection.recordMap?.notion_user
  )
